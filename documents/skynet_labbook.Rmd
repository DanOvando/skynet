...
# Why are unfished easier to predict?

Well, it's not because they are less variable... they are more variable. 


```{r}
a = candidate_data %>%
  filter((fished_only == TRUE |
           unfished_only == T) & survey_months_only == TRUE) %>% 
select(fished_only, skynet_data) %>% 
  unnest()

a %>% 
  ggplot(aes(log_density, fill = fished_only)) + 
  geom_histogram()

a %>% 
  group_by(fished_only) %>% 
  summarise(vd = sd(log_density))
```
...
fish_abundance <- a %>% 
  ...
  group_by(survey,knot, rounded_lat, rounded_lon, fished_only) %>% 
  ...
  ggplot() +
  geom_sf(aes(color = total_density), size = 2, alpha = 0.5) +
  # geom_sf(data = pacific_map, fill = 'grey60') +
  # coord_sf(xlim = c(bbox['xmin'], bbox['xmax']),
  # ylim = c(bbox['ymin'], bbox['ymax'])) +
...
If this thing is to be applied, need to pick a way to do it, since you won't know the specifics of a new location per say. So one option is to do a lot of kitchen sink presentation, but that's annoying in my humble opinion. So, one option is to narrow down to the "best" model and just present that one for most of the paper. Another is to identify "best" for each task (future trend prediction/absolute abundance/spatial distribution) The things to choose from are basically model, dependent variable, variable set, and geographic scale. 

First, look at what's the "best" dependent variable, including "delta" as the thing being predicted. Once you've picked that, then look at the performance of that 

Not a lot of evidence that any one of these is particularly better than another, so might be best to just stick with density as it's the simplest thing to measure. Though also need to make sure that once you exponentiate it still holds up as the "best" since that's what would actually be used on management scale. 

```{r}

skynet_models %>% 
  ggplot(aes(dep_var, r2)) + 
  geom_boxplot() + 
  facet_grid(data_subset ~test_set) + 
  coord_flip()
...
log_check <- skynet_models %>%
  filter(dep_var == "log_density",
  data_subset == "skynet",
  train_set == "random_alaska",
  model == "gbm",
...
log_plot <- log_check %>% 
  ...
  labs(title = glue::glue("R2 is {yardstick::rsq(log_check, log_density, pred) %>% round(2)}"))
  

linear_plot <- log_check %>% 
  ggplot(aes(density, exp_pred)) + 
  geom_point() + 
    geom_abline(aes(intercept = 0, slope = 1), linetype = 2, color = "red") +
  labs(title = glue::glue("R2 is {yardstick::rsq(log_check, density, exp_pred) %>% round(2)}")) + 
  lims(x = c(0,600))
...
log_check <- skynet_models %>%
  filter(dep_var == "log_economic_density",
  data_subset == "skynet",
  train_set == "random_alaska",
  model == "gbm",
  weight_surveys == FALSE,
...
linear_check <- skynet_models %>%
  filter(dep_var == "economic_density",
  data_subset == "skynet",
  train_set == "random_alaska",
  model == "gbm",
  weight_surveys == FALSE,
...
log_plot <- log_check %>% 
  ggplot(aes(log_economic_density, pred)) + 
  geom_point() + 
  geom_abline(aes(intercept = 0, slope = 1), linetype = 2, color = "red") +
  labs(title = glue::glue("R2 is {yardstick::rsq(log_check, log_economic_density, pred) %>% round(2)}"))
  

linear_plot <- log_check %>% 
  ggplot(aes(economic_density, exp_pred)) + 
  geom_point() + 
    geom_abline(aes(intercept = 0, slope = 1), linetype = 2, color = "red") +
  labs(title = glue::glue("R2 is {yardstick::rsq(log_check, economic_density, exp_pred) %>% round(2)}")) 

true_linear_plot <- linear_check %>% 
  ggplot(aes(economic_density, pred)) + 
  geom_point() + 
    geom_abline(aes(intercept = 0, slope = 1), linetype = 2, color = "red") +
  labs(title = glue::glue("R2 is {yardstick::rsq(linear_check, economic_density, pred) %>% round(2)}")) 

log_plot + linear_plot + true_linear_plot
  
```

Oh interesting. Just running on the damn raw stuff does better then transforming. Let's do a quick check if that holds true for the raw density not just economic. 
...
true_linear_plot <- linear_check %>% 
  ...
  geom_point() + 
    geom_abline(aes(intercept = 0, slope = 1), linetype = 2, color = "red") +
  labs(title = glue::glue("R2 is {yardstick::rsq(linear_check,density, pred) %>% round(2)}")) 

log_plot + linear_plot + true_linear_plot

```

Evidence then is that just fitting on the damn thing you're interested in is the best option.. interesting. Let's check 

# model comparison

So, you've got things working much better now. Still leaves a question of deciding which model to use. So far, you're just using RMSE/R2 for model selection. Should you be using tidyposterior for this, or is that just overkill?

Or more specifically, do you need to run v-fold cross validation on the tuned model to then generate a distribution of predictive power? Seems like a pain in the arse with little payoff from comittee... though it is the right way to do it. But the idea here would be to use once set of cross cv to fit the effing model. So for each model you fit it, but you also create a bunch of bootstrapped/cross fold samples of the training data and use that to assess model performance. Let's do a speed test I guess...

Looks like you can pass vfold_cv objects to perf_mod... not a bad idea. So you would then Take the model... and train it and predict it on each split, then calculate statistic like RMSE, and use that to compare the damn models. woof. Let's see how hard it would be... 

```{r}

    mars_foo <- function(data){
      
      
      prepped_recipe <- recipes::prep(train_recipe, rsample::training(data), retain = T)
      ...
      proc_testing <- bake(prepped_recipe, rsample::testing(data))
      
      
      pred <- predict(model, newdata = proc_testing)
      
      proc_testing <- proc_testing %>% 
        mutate(log_pred = pred ) %>% 
        mutate(resid = dep_var)
    ...
    kfold_training <- rsample::vfold_cv(training, v = 10)
    
```


Ranger is fitting by sampling the data with replacement, so already creating a bunch of synthetic datasets to generate the model fitting, and using OOB for the actual predictions. 

SHOOT. seems like there's something broken with pred + recipes + gbm, which makes the whole thing fairly useless, unless you add it in yourself. Which seems like the right way to go. 

But, not really any need to do that for every thing. That would be a 5-fold increase in run time (roughly) to run it for each model. 

So instead, write a function that does a kfold cross validation of the final model on demand, and do it across a few metrics (random, spatial, time)

# paper revamp

OK, here's the new structure

1. Background etc

2. Which model is best? 
  - Based off of training performance, not testing performance. 
  - Assess impact on spatial scale on training performance in this step. 
  - Do a smaller analysis using tidyposterior to dial down between candidate top models
  - This is the model that evidence suggests has best chance to do well out of sample
  - focus on normal density, have economic density as backpocket to show that performance isn't that different, and that actual density is slightly more fisheries management relevant. 
3. What does the best model say?

*OOB preformance will always be worse, so while machine learning might get worse, structural won't get better, and since best bet of performance is too low to be useful in the first place, so good reason to move on to machine learning and focus on model comparison there. 

- Now, evaluate testing performance of JUST the best model

4. Appendix
- back up showing performance of other models to show that you're not missing the bus here

scale residuals by standard deviation
...
kfold_preds_summary <-  kfold_preds %>% 
  map_df(~., .id = "model") %>% 
  select(Resample, model, obs, pred,) %>% 
...
Now, you also need to just pick one damn data subset. So after you pick the best model, you should pick the best data subset, across the range of test sets, holding variables constant (include all variables, not just GFW? or maybe just with GFW? check on that?). Acknowldege that different models might be best for different purposes, and this framework could be repurposed to deal with that, but for now goal is simply find the overall best model, in terms of mechanics, data transformations, GFW only, etc. 


# breaking up the west coast

For purely aesthetic reasons, could be interesting to break the west coast into regions a bit and see if that gives better resolution to the temporal trends 


```{r}

knots_per_survey <- tribble(washington)

a = vast_fish %>% 
  select(survey, vasterized_data) %>% 
  mutate(vast_fish = map(vasterized_data, "spatial_densities")) %>% 
  select(-vasterized_data) %>% 
  unnest() %>% 
  filter(survey == "wcgbts") %>% 
  mutate(state = if_else(approx_lat > 46.25, "washington", if_else(approx_lat > 42, "oregon", "california")))
...
Interesting, lookg like it could be worthwhile, apparent doward trends in california and washington , upward in oregon


# what is it about delta skynet?

THere are like 2 scenarios where delta skynet is doing great out of sample... what is that?

```{r}

best_fits %>% 
  filter(dep_var == "density", gfw_only == TRUE) %>% 
  gather(r2_source, r2_value, r2, r2_training) %>% 
  ggplot(aes(data_subset,r2_value, color = r2_source)) + 
  geom_point(position = "dodge") + 
  coord_flip()


a = best_fits %>% 
  filter(dep_var == "density", gfw_only == TRUE, data_subset == "delta_skynet") %>% 
...
Interesting, huge discrepency between the test and training. lag1 skynet is stupid; it's jsut skynet but with lagged engine power as a candidate variable. 
...
train_performance <- best_fits %>% 
  ...
  mutate(split = "Training") %>% 
    mutate(state = ifelse(survey == "wcgbts",if_else(rounded_lat > 46.25, "washington", if_else(rounded_lat > 42, "oregon", "california")),"Alaska"))
...
performance <- train_performance %>% 
  ...
  group_by(state, year, split) %>% 
  summarise(observed = sum(density * 100),
            predicted= sum(pred * 100)) %>% 
  gather(source, value, observed:predicted) %>% 
  ungroup() %>% 
  group_by(state, split, source) %>% 
  mutate(cs_value = (value - mean(value)))

performance %>% 
  ggplot(aes(year,cs_value, color = state, linetype = source)) + 
  geom_line() + 
  scale_color_discrete(guide = FALSE) +
  facet_grid(state~split, scales = "free_y")

```

# what's up with structural?

Seems like it got a lot worse, worth checking to make sure it still works at all

Ok one point of confusion that I had before is what the hell that second q is doing inside the miller and deacon paper , qsB*exp(-qE), where S is selectivity or really proportion of the biomass vulnerable to gear and q is the catchability coefficient of those vulnerable individuals. qsB is aproximated by CPUE in the last time period. THe key here is that they aren't modeling the population, they'r emodeling the trawl vulnerable population, which is what they base fishermen's expectations on, which seems reasonable. 

oh right, miller inverts things that's where that negative dissapeared to. 
...
a %>% 
  ggplot(aes(log_density, log(pred))) + 
  geom_point() 
...
So this valus of information thing is really really really not good on any level. 

Looking at the ranger results, it seems like adding the GFW data to the mix is actually making things worse somehow, which I wouldn't think should be possible! If the GFW variables don't matter, they just shouldn't get selected. Part of the issue there seems to have been model tuning, in that the same tuning does not apply to each subset of variables, so you need to fix that. But, in the test-tuning script, you used the best tuning for each variable set, and still got that result with ranger... 

but not with GBM it seems... at least there adding in GFW provides some marginal benefits, but seriously marginal. So if that holds up, suggests that something is actually breaking in ranger, which is a bit scary from a model selection point of view. How do you justify which model to use around "because it doesn't makes sense for it to get worse"?
...
ps <-
  foreign::read.dbf(here::here("data","purse_seine_4","PURSE_SEINE.DBF")) %>%
  set_names(tolower(colnames(.))) %>%
  as_data_frame() %>%
...
ps_bbox <- c(min(ps$lat), max(ps$lat), min(ps$lon), max(ps$lon))
...
gfw_query <- glue::glue_sql(
  ...
  *
  ...
  AND lon > ({bbox[3]})
  AND lon <= ({bbox[4]})
  AND _PARTITIONTIME BETWEEN TIMESTAMP('2014-01-01')
...
gfw_wcpfc <-
  bigrquery::query_exec(gfw_query, project = "ucsb-gfw", max_pages = Inf, destination_table = "skynet.wcpfc") 
...
dbListTables(fishing_connection)

gfw_wcpfc <- tbl(fishing_connection, "wcpfc")


test <- gfw_wcpfc %>% 
  filter(a_total_hours > 0 & is.na(a_total_hours) == FALSE) %>% 
  select(-b_mmsi, -b_year) %>%
  select(a_mmsi, a_year,a_month,a_day, b_inferred_label_allyears,
         b_known_label) %>% 
  group_by(a_mmsi, a_year, a_month, a_day) %>% 
  summarise(
    inferred_label_allyears = unique(b_inferred_label_allyears)) %>% 
  collect()


woop <- test %>% 
  filter(is.na(inferred_label_allyears) == F) %>% 
  mutate(date = lubridate::ymd(glue::glue("{a_year}-{a_month}-{a_day}"))) %>%
  filter(inferred_label_allyears == "purse_seines"
         ) %>% 
  group_by(a_year, a_mmsi) %>% 
  summarise(mmsi_days = n_distinct(date)) %>% 
  group_by(a_year) %>% 
  summarise(total_days = sum(mmsi_days)) %>% 
  mutate(source = "gfw")


ps_days %>% 
  filter(year > 2000) %>% 
  bind_rows(woop %>% rename(year = a_year)) %>% 
  ggplot(aes(year, total_days, color = source)) + 
  geom_point()
...
# drivers of fishing location

Idea: using each vessel as input, put together a decision tree model that estimates whether or not to fish at a given location. 

Could also do a logit model to that effect, and a RUM model if you wanted? Explore the easy options first. Or just a GLM model predicting effort conditional on choosing to fish somewhere. 

# comparing to FishData

Rather than doing a bunch of diagnostics on VASt, let's compare your estimates for a few species and see if the maps and trends look more or less the same! smart. 

```{r}

compare <- vast_fish %>% 
  filter(survey == "wcgbts",
         spp == "Squalus_suckleyi")

compare$vasterized_data[[1]]$spatial_densities %>% 
  group_by(year) %>% 
  summarise(total_biomass = sum(biomass)) %>% 
  ggplot(aes(year, total_biomass)) + 
  geom_line() +
  geom_point()

```

temporal trends match fishdata perfectly. 
...
a = compare$vasterized_data[[1]]$spatial_densities %>% 
  filter(year == 2010)
...
alaska_bbox <- sf::st_bbox(spatial_performance_plot)



alaska_spatial_residual_plot <- ggplot() +
  geom_sf(data = spatial_performance_plot,
  aes(color = log(density)), size = 1) +
  geom_sf(data = pacific_map, shape = 21) +
  coord_sf(
  xlim = c(alaska_bbox['xmin'], alaska_bbox['xmax']),
  ylim = c(alaska_bbox['ymin'] * .95, alaska_bbox['ymax'] * 1.1),
  expand = F
  ) +
...
alaska_spatial_residual_plot



```

OK, can thoroughly confirm that your results match up perfectly with fishdata. So there may be a vast problem, but it's not related to how you're choosing to run it. 

# predicting effort

```{r}

a = candidate_data$skynet_data[[1]] 
...
test <- stan_glm(any_fishing ~ dist_from_port + dist_from_shore +no_take_mpa + restricted_use_mpa + density +  mean_analysed_sst +  mean_chlorophyll, data = a, family = binomial)
...
data <- a %>% 
    filter(survey == "wcgbts", year == 2015, any_fishing == T) %>% 
  select(total_hours, dist_from_port,dist_from_shore,
         no_take_mpa,restricted_use_mpa, density, mean_analysed_sst,
        mean_chlorophyll ) 

fish_recipe <- recipe(total_hours ~ . , data = data) %>%
  step_center(all_predictors(),-no_take_mpa, -restricted_use_mpa) %>%
  step_scale(all_predictors(),-no_take_mpa, -restricted_use_mpa) %>% 
...
test <- stan_glm(total_hours ~ ., data = prepped_data, family = gaussian)
...
woah, there's a LOT of fishing going on in no take mpas...
...
a %>% 
  filter(survey == "wcgbts", year == 2015) %>% 
  group_by(no_take_mpa) %>% 
  summarise(prop_fishing = mean(any_fishing),
            total_hours = sum(total_hours))

a %>% 
  filter(year == max())
  group_by(any_fishing) %>% 
...
qmplot(x = rounded_lon, y = rounded_lat, color = no_take_mpa, data = b %>% filter(rounded_lon > -175)) + 
  theme_minimal() + 
  facet_wrap(~year)

```

can confirm that dep var doesn't affect tuning parameters. 

# flipping the equation 

Testing one last final damn thing. Let's flip the equation on its head and estimate random effects for each knot to predict effort and see what that does. 
...
a <- candidate_data %>% 
  filter(fished_only == TRUE, unfished_only == FALSE, survey_months_only == TRUE)

data <- a$skynet_data[[1]] %>% 
  filter(survey == "wcgbts")

reg_data <-  data %>% 
  select(total_hours, mean_chlorophyll, mean_analysed_sst, dist_from_port, dist_from_shore, m_below_sea_level, knot, year) %>% 
  mutate(knot = as.factor(knot),
         year = as.factor(year))

reg_recipe <- recipe(total_hours ~ ., data = reg_data %>% filter(total_hours >0)) %>%
  step_center(all_numeric(), -all_outcomes()) %>% 
  step_scale(all_numeric(), -all_outcomes()) %>% 
  step_log(all_outcomes())
  
reg_data <- prep(reg_recipe, data = reg_data, retain = T) %>% juice()


effort_model <- stan_glmer(total_hours ~ mean_chlorophyll + mean_analysed_sst + dist_from_port + dist_from_shore + m_below_sea_level + (1 |knot:year), data = reg_data, chains = 4,
                           iter = 5000)


effort_coefs <- effort_model$coefficients %>% 
  tidy() 

intercept <- effort_coefs$x[effort_coefs$names == "(Intercept)"]

knots <- effort_coefs %>% 
  filter(str_detect(tolower(names),"knot")) %>% 
  mutate(knot_year = str_replace(names, ".*(?<=year:)",""))
  
  knot_years <- str_split(knots$knot_year, ":", simplify = T) %>% 
    as_data_frame() %>% 
    map_df(~str_replace_all(.x, "\\D","")) %>% 
    map_df(as.numeric) %>% 
    set_names("knot", "year")
  
  knots <- knots %>% 
    bind_cols(knot_years) %>% 
  mutate(knot_effect = x)

data <- data %>% 
  left_join(knots, by = c("knot","year"))

data %>% 
  ungroup() %>% 
  mutate(log_density = log_density / max(log_density, na.rm = T),
         knot_effect = knot_effect / max(knot_effect, na.rm = T)) %>% 
  ggplot(aes(log_density, knot_effect)) + 
  geom_abline(aes(intercept = 0, slope = 1), linetype = 2, color = "red") +
  geom_point() + 
  geom_smooth(method = "lm") +
  facet_wrap(~year)
...
yardstick::rsq(data, log_density, knot_effect)

```

Not much there. Last effort here, 
...
Nothing doing there. OK, so interesting idea, mention it in discussion, but not worth fleshing out at this time
...
nmfs_catch <- nmfs_landings %>% 
  filter(state %in% c("California","Washington", "Oregon"), scientific_name %in% included_species) %>% 
  group_by(year) %>% 
  summarise(total_nmfs_catch = sum(metric_tons, na.rm = T))


fao_catch <- fao_capture %>% 
  filter(fao_area_code == 77, scientific_name %in% included_species) %>% 
  group_by(year) %>% 
  summarise(total_fao_catch = sum(capture, na.rm = T))
...
ebsbts_catch <- ram_catch %>% 
  ...
  group_by(year) %>% 
  summarise(total_catch = sum(catch, na.rm = T),
            n_catch_stocks = sum(is.na(catch) == F & catch > 0))
...
ebsbts_biomass<- ram_biomass %>% 
  ...
  group_by(year) %>% 
  summarise(total_biomass = sum(biomass, na.rm = T),
            n_biomass_stocks =sum(is.na(biomass) == F & biomass > 0))
...
What do effort dynanmics tell us about fish abundance?

- On a macro scale fishing effort has to respond to fish abundance (0 fish no fishing)

- Now that we can observe fine scale fishing effort, what does this tell us?

# Evidence

- Effort vs biomass

- Effort / CPUE vs biomass

- Conditional on GFW only... machien learning and structural approach

- Value of information

# Discussion

So what then. If all results hold that simply not a useful predictor... what does that say? 

- Choice of where and how much to fish is complicated as hell (defense from more localized assessments?)

- Beware of simple bio-economic models?

- e-catch data to pair with GFW style information?


# what's up with the knots?
...
wtf$vasterized_data[[1]]$spatial_densities -> a

basic_skynet_data %>% 
  filter(survey == "ebsbts") %>% 
  ggplot(aes(rounded_lon, rounded_lat, label = knot)) + 
  geom_text(show.legend = F, size = 2) + 
  coord_cartesian(ylim = c(45,50), xlim = c(-126, -123))


knots %>% 
  filter(survey == "wcgbts") %>% 
  unnest() %>% 
  ggplot(aes(approx_long, approx_lat, label = knot)) + 
  geom_text(show.legend = F, size = 2) + 
  coord_cartesian(ylim = c(45,50), xlim = c(-126, -123))

basic_skynet_data %>% 
  group_by(survey) %>% 
  select(survey, rounded_lat, rounded_lon) %>% 
  unique() %>% 
  group_by(survey) %>% 
  count()
...
a = gfw_data %>% 
  unnest() %>% 
  filter(survey == "wcgbts") %>% 
  group_by(survey, rounded_lat, rounded_lon) %>% 
  summarise(te = sum(total_hours))
...
Look into spatial residuals, Bryan Stock has done some work on that. Residuals


# better paper framing

the paper is still about predicting, but instea of "it doesn't work", the story is that the environment is a more perfect predictor, and the data suggests that fishermen are responding to the environment, but constrained by regulation. So, environmental proxies for distributions in trends might be a more worthy path to pursue

Environment is the only thing that moves

ARE BEHAVIOR AND THE ENVIRONMENT WORTH MORE THAN THE SUM OF THEIR PARTS?

# idiota

So wait, you went back and for the paper remade the just effort vs density plot and noticed something. If you correctly sum up to total effort vs total biomass (or total revenue), then there seems to be a pretty damn clear relationahip between total effort and total revenue, i.e. the alaska hypothesis. 

And, if that's the case, just eyeballing it, on the log scale a simple linear model would do quite well in or out of sample. So, a more complex model had damn well better do better as well. 

So part of the problem was that you were doing total effort vs mean density over an area, and that doesn't really make any sense. If the bulk of the fishing is in a small area and you sum over a bigger area, but then average over the densities, the mean density could be low even if the total density is low. 

You have to either do total vs totals or means vs means, or at least those seem a lot more logical. And if that's the case, it certainly seems like this thing might actually work. Still no guarantee that it would be better or worse than just ent environment, but still way better to explore. POOP. 


```{r}

skynet_data <- basic_skynet_data

downscaled_performance <-
  cross_df(list(
    resolution = c(2,25,200),
...
e_v_ed_plot <- downscaled_performance %>%
  ...
  ggplot(aes((agg_mean_density), (total_revenue), fill = approx_survey)) +
...
starting on page three, where you say there's not a lot of research linking abundance to effort, cite Ray's work on this, see his attached papers

His main interest: does effort change with abundance?

TO get at figure 6 point, maybe show total summed effort and total available revenues by region to make clearer that when you sum the plots there's more fishing where there's more effort?

For the statistical analysis then, you can compare the trends from GFW only to GFW+enviro + enviro only

Yup! 

Maybe make the paper a little clearer at first that you tried adn the answer was no?

# UNITS AGAIN

Let's go through and check all your units one. last. damn. time. 

```{r}
vast_fish$vasterized_data[[1]]$spatial_densities$density %>%
  hist()

```

You know that it's at least in something per km^2. Now, what are these numbers? I'm guessing metric tons. If it's kg, then the max density per km^2^ in the EBS is like 1700 lbs. 800 tons per square KM makes a lot more sense. 

Hmmm, but wait

Index_xcyl(x,c,y,l) = D_xcy(x,c,y) * a_xl(x,l) / 1000;  // Convert from kg to metric tonnes

so this would seem to suggest densities here are actually in kg, not in tons. 

Let's compare the time index to the sum of the biomasses and see if the indicies are approx 1/1000


```{r}
ddensity <- vast_fish$vasterized_data[[43]]$spatial_densities %>% 
  group_by(year) %>% 
  summarise(biomass = sum(density * area)) %>% 
  mutate(source = "density")


idensity <- vast_fish$vasterized_data[[43]]$time_index %>% 
  rename(year = Year) %>% 
  group_by(year) %>% 
...
ddensity %>% 
  bind_rows(idensity) %>% 
  ggplot(aes(year, biomass, color = source)) + 
  geom_jitter() 
...
identical. I see. Well, on one level, there has to be more than 70,000 kf of alaska pollock in the EBSBTS... so has to be tons. though now even that seems low. 

Projected just ssb in 2018 of walleye pollock is 5,831,610

AHA. The damn problem is in your area swept calculations. You have them all set to 1, which sure as shit isn't right. 

From the VAST examples

it seems like .01 is the default for EBS and GOA, AI is... 1, WCGBTS is... unclear?

WCGBTS_Canary_example[,"AREA_SWEPT_HA"]/1e2

So at the very least that needs fixing to get your units in line here. A quick example of taking the biomasses and dividing by .01 seems to get you into the right area. 
...
none %>% 
  bind_rows(one) %>% 
  group_by(survey, variables, year,scale) %>% 
  summarise(tb = sum(unique(biomass))) %>% 
  ggplot(aes(year, tb, fill = scale)) + 
  geom_col(position = "dodge") + 
  facet_grid(survey ~ variables, scales = "free")
...
one <- data_sources$data[data_sources$data_subset == "skynet_100km"][[1]] %>% 
    mutate(scale = "one") %>% 
    select(-no_take_mpa,-restricted_use_mpa)




none <- data_sources$data[data_sources$data_subset == "skynet"][[1]] %>% 
    mutate(scale = "none") %>% 
  select(-no_take_mpa,-restricted_use_mpa)

none %>% 
  bind_rows(one) %>% 
  group_by(survey, year,scale) %>% 
  summarise(tb = sum(unique(biomass))) %>% 
  ggplot(aes(year, tb, fill = scale)) + 
  geom_col(position = "dodge") + 
...
OK, so now can confirm, we're in VAST units of tons per km^2. thank god. And the price database is in units of $ per ton, so no need for conversions there. 

Lastly, what's up with the really low biomasses in some places. 
...
So is the problem the cell resolution, or is it in the number of effective knots?

```{r}

check <- vast_fish %>% 
  mutate(spatial_abundance = map(vasterized_data, "spatial_densities")) %>% 
  select(-data, -vasterized_data) %>% 
  mutate(vast_worked = map(spatial_abundance,dim)) %>%
  mutate(vast_worked = !map_lgl(vast_worked, is_null)) %>% 
  filter(vast_worked) %>% 
  unnest()
...
hake is about 1.5 million, so, if that'as 1/5 of the total biomass of fishable species out there, that's not totally craz...., and that's counting canada
