...
trawl_fishing_by_knot <- ebs %>% 
  ...
  summarise(total_hours = sum(total_hours, na.rm = T),
            total_engine_hours = sum(total_hours * inferred_engine_power, na.rm = T),
...
skynet_model <- randomForest::randomForest(density ~ ., data = skynet_data %>% select(-e_km, -n_km, -knot,-year))
...
$\pi_{i,j} = p*cpue_{j}*e^{-q_{j}T_{i,j}} - \beta_{j}*distance_{i}$
...
$[q_{j}, \beta_{j} | cpue_{j}] \propto [cpue_{j} | q_{j},\beta_{j}][q_{j}][\beta_{j}]$

Where you can assign prior to beta_j based on fuel prices and labor costs. 


So, now you need to think about how you would estiamate each of these parameters. Let's make a list of all the parameters you need for this model, and how you'd go about estimating them. 
