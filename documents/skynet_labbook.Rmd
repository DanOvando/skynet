...
resolution_test <- skynet_models %>% 
  filter(data_subset == "skynet", dep_var == "log_density") %>% 
...
mapfoo <- function(data){
  ...
  data_map <-  data %>%
  dplyr::mutate(geometry = purrr::map2(approx_long, approx_lat, ~ sf::st_point(x = c(.x, .y), dim = 'XY'))) %>%
  ungroup() %>%
  mutate(geometry = sf::st_sfc(geometry, crs =
                                 "+proj=longlat +datum=WGS84 +no_defs")) %>%
  sf::st_sf()
...
data_map <- data_map %>%
  ggplot() +
  geom_raster(aes(x = approx_long, y = approx_lat, fill = log(total_density)), show.legend = F, interpolate = F) +
