...
skynet_models %>% 
  ggplot(aes(r2_training, fill = unfished_only)) +
  geom_histogram() +
  facet_grid(dep_var ~ model)
...
best_models %>%
  ...
  ungroup() %>% 
  mutate(test_sets = fct_reorder(test_sets, max_r2)) %>% 
  ggplot(aes(test_sets,r2)) + 
  geom_line() +
...
```{r}
spatial_performance <- best_models %>% 
  filter(test_set == "random", dep_var == "density")  %>%
  filter(r2 == max(r2, na.rm = T)) %>% {
    .$training_data[[1]]
  } %>% 
...
train_performance <- skynet_models %>% 
  filter(test_set == "random_alaska", dep_var == "log_density", data_subset == "skynet")  %>%
  filter(r2 == max(r2, na.rm = T)) %>% {
    .$training_data[[1]]
  } %>% 
