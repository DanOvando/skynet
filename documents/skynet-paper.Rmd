...
upscale_plot <- skynet_models %>% 
  ungroup() %>% 
  filter(test_sets == "random", str_detect(data_subset,"skynet"), dep_var == "density",!str_detect(data_subset,"delta"),
...
best_fits %>% 
  ...
  ggplot(aes(test_set, r2, fill = model)) + 
...
train_performance <- best_fits %>% 
  filter(test_set == "random_alaska", dep_var == "density", data_subset == "skynet")  %>%
  filter(r2 == max(r2, na.rm = T)) %>% {
    .$training_data[[1]]
  } %>% 
...
train_performance <- skynet_models %>% 
  filter(test_set == "west_coast", dep_var == "density", data_subset == "skynet_100km")  %>%
  filter(r2 == max(r2, na.rm = T)) %>% {
    .$training_data[[1]]
  } %>% 
  ungroup() %>% 
...
train_performance <- best_fits %>% 
  filter(test_set == "random", dep_var == "density", data_subset == "skynet_25km")  %>%
  filter(r2 == max(r2, na.rm = T)) %>% {
    .$training_data[[1]]
  } %>% 
  mutate(resid = pred - density) %>% 
...
time_train_performance <- skynet_models %>% 
  filter(test_set == "year_greq_than_2014", dep_var == "density",data_subset == "skynet_25km")  %>%  filter(r2 == max(r2, na.rm = T)) %>% {
    .$training_data[[1]]
  } %>% 
  mutate(resid = pred - density) %>% 
...
time_test_performance <- skynet_models %>% 
  filter(test_set == "year_greq_than_2014", dep_var == "density",data_subset == "skynet_25km")  %>%
  filter(r2 == max(r2, na.rm = T)) %>% {
    .$test_data[[1]]
  } %>% 
  mutate(resid = pred - density) %>% 
