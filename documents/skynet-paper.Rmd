...
fig_theme <- theme_get() + 
  theme(axis.text = element_text(family = "comic-sans"))
...
a = skynet_models %>% 
  ...
  mutate(model = fct_reorder(model, mean_r2)) %>% 
  filter(model == "gbm") %>% 
  filter(mean_r2 == max(mean_r2)) %>%
  group_by(test_set) %>% 
  mutate(mean_r2 = mean(r2, na.rm = T)) %>% 
  ungroup() %>% 
  mutate(test_set = fct_reorder(test_set, -mean_r2)) %>% 
  select(dep_var, weight_surveys, test_sets, data_subset, test_data) %>% 
  mutate(obs_pred = map2(test_data,dep_var, ~.x %>% select(.y,pred))) %>% 
  select(-test_data) %>% 
  unnest() %>% 
  gather(observed_name, observed,contains("log"), contains("density")) %>% 
  ggplot(aes(observed, pred, fill = weight_surveys)) + 
  geom_point(shape = 21) + 
  geom_abline(aes(intercept = 0, slope = 1), linetype = 2, color = "pink") +
  facet_wrap(dep_var ~ test_sets, scales = "free") + 
  theme_minimal()

a

huh <- skynet_models %>% 
  filter(test_sets == "alaska")
```
...
upscale_plot <- skynet_models %>% 
  ungroup() %>% 
  filter(test_sets == "random", str_detect(data_subset,"skynet"), dep_var == "log_density",!str_detect(data_subset,"delta")) %>% 
  group_by(model) %>% 
...
spatial_performance <- skynet_models %>% 
  ...
  mutate(resid = pmax(-200,exp(pred) - exp(log_density)))
```
...
map_theme +
  ...
  resid_hist + spatial_residual_plot + plot_layout(ncol = 2, nrow = 1, widths = c(1,2),heights = c(1,2))
```
...
```{r rel-plot, fig.cap = "Spatial residuals of relative (density divided by max density) density predicted by GBM model (log(density) is dependent variable). Negative residuals (red) indicate model underpredicted rank of the true density, positive residuals (blue) indicate the model overpredicted. Data aggregated at 100km^2^ resolution"}
...
train_performance <- skynet_models %>% 
  filter(test_set == "random", dep_var == "log_density", data_subset == "skynet_25km")  %>%
  filter(r2 == max(r2, na.rm = T)) %>% {
    .$training_data[[1]]
  } %>% 
  mutate(resid = exp(pred) - exp(log_density)) %>% 
...
performance %>% 
  ...
  facet_grid(survey~split, scales = "free_y")
...
time_train_performance <- skynet_models %>% 
  filter(test_set == "year_greq_than_2014", dep_var == "log_density",data_subset == "skynet_25km")  %>%  filter(r2 == max(r2, na.rm = T)) %>% {
    .$training_data[[1]]
  } %>% 
  mutate(resid = pred - log_density) %>% 
...
time_test_performance <- skynet_models %>% 
  filter(test_set == "year_greq_than_2014", dep_var == "log_density",data_subset == "skynet_25km")  %>%
  filter(r2 == max(r2, na.rm = T)) %>% {
    .$test_data[[1]]
  } %>% 
  mutate(resid = pred - log_density) %>% 
