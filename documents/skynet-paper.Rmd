...
skynet_subset <- skynet_models %>% 
  select(model, data_subset, fitted_model, train_set) %>% 
  mutate(results = map(fitted_model,'result')) %>% 
  mutate(worked = !map_lgl(results, is.null)) %>% 
  filter(worked == T)

skynet_subset_training <- skynet_subset %>% 
  mutate(train_results = map(results,'training_predictions')) %>% 
  select(model, data_subset,train_set, train_results) %>% 
  unnest() %>% 
...
skynet_subset <- skynet_models %>% 
  ...
  select(model, data_subset, fitted_model) %>% 
  mutate(results = map(fitted_model,'result')) %>% 
  mutate(worked = !map_lgl(results, is.null)) %>% 
  filter(worked == T)
...
skynet_subset_training <- skynet_subset %>% 
  mutate(train_results = map(results,'training_predictions')) %>% 
  select(model, data_subset, train_results) %>% 
  unnest() %>% 
  mutate(residuals = log_density - pred)

skynet_subset_testing <- skynet_subset %>% 
  mutate(train_results = map(results,'test_predictions')) %>% 
  select(model, data_subset, train_results) %>% 
  unnest() %>% 
  mutate(residuals = log_density - pred)
```
...
varImpPlot(skynet_models$fitted_model[[1]]$result$model$finalModel)
...
```{r perf-plot, fig.cap='Observed vs predicted log CPUE for test data from models trained on training data', eval=F}

performance_plot

```

```{r rmse-plot, fig.cap= 'RMSE and R2 of each model on test data', eval = F}
rmse_plot
```
...
```{r time-perf-plot, fig.cap='Observed vs predicted log CPUE for test data (2015) from models trained on training data (pre 2015)', eval = F}
time_performance_plot
```



```{r time-rmse-plot, fig.cap= 'RMSE and R2 of each model on test (2015) data', eval = F}
time_rmse_plot
```
