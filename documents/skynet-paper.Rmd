...
spatial_residual_plot <- ggplot() + 
  geom_sf(data = spatial_performance_plot, aes(color = scaled_resid)) + 
  geom_sf(data = pacific_map, shape = 21) + 
  coord_sf(xlim = c(bbox['xmin'], bbox['xmax']),
  ylim = c(bbox['ymin']*.75, bbox['ymax'] * 1.1), expand = F) + 
  scale_color_gradient2(low = "tomato", high = "steelblue",midpoint = 0, mid = "white", name = "Residuals") + 
map_theme +
  labs(title = "B)")

alaska_spatial_residual_plot <- ggplot() + 
  geom_sf(data = spatial_performance_plot %>% filter(survey != "wcgbts"), aes(color = scaled_resid)) + 
  geom_sf(data = pacific_map, shape = 21) + 
  coord_sf(xlim = c(alaska_bbox['xmin'], alaska_bbox['xmax']),
  ylim = c(alaska_bbox['ymin']*.95, alaska_bbox['ymax'] * 1.1), expand = F) + 
  scale_color_gradient2(low = "tomato", high = "steelblue",midpoint = 0, mid = "grey", name = "Residuals", guide = guide_colorbar(frame.colour = "black")) + 
map_theme +
  labs(title = "Alaska") + 
...
(wc_spatial_residual_plot + alaska_spatial_residual_plot) / resid_hist  + plot_layout(nrow = 2, ncol = 1, widths = c(2,1),heights = c(2.5,1))
```
...
train_performance <- best_fits %>% 
  filter(test_set == "random_alaska", data_subset == "skynet")  %>%{
    .$training_data[[1]]
  } %>% 
...
time_train_performance <- skynet_models %>% 
  ...
  mutate(resid = pred - density) %>% 
      mutate(state = ifelse(survey == "wcgbts",if_else(rounded_lat > 46.25, "washington", if_else(rounded_lat > 42, "oregon", "california")),"Alaska")) %>% 
  group_by(state, year) %>% 
  summarise(observed = mean(density * 100), 
            predicted =  mean(pred * 100)) %>% 
  gather(source,value, observed:predicted) %>% 
  ungroup() %>% 
  mutate(split = "training")
...
time_test_performance %>% 
  ...
  ggplot(aes(year, cs_value, color = source)) + 
