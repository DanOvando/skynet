...
a = skynet_models %>% 
  ...
  ungroup() %>% 
  mutate(model = fct_reorder(model, mean_r2)) %>% 
  filter(mean_r2 == max(mean_r2), dep_var == "log_density") %>% 
  group_by(test_set) %>% 
  mutate(mean_r2 = mean(r2, na.rm = T)) %>% 
  ungroup() %>% 
  mutate(test_set = fct_reorder(test_set, -mean_r2)) %>% 
  select(dep_var, weight_surveys, test_sets, data_subset, training_data) %>% 
  mutate(obs_pred = map2(training_data,dep_var, ~.x %>% select(.y,pred))) %>% 
  select(-training_data) %>% 
  unnest() %>% 
  gather(observed_name, observed,contains("log")) %>% 
