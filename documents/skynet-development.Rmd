...
long_2_utm <- function(long) {
    (floor((long + 180)/6) %% 60) + 1
  
  # https://stackoverflow.com/questions/9186496/determining-utm-zone-to-convert-from-longitude-latitude
}

projfoo <- function(lon,lat,zone){

  project(cbind(lon, lat), paste0("+proj=utm +zone=",zone," ellps=WGS84")) %>% as.data.frame() %>% 
    set_names(c('easting','northing'))
}

ebs_locs <- ebs_locs %>% 
  mutate(utm_zone = long2UTM(rounded_lon)) %>%
  {
  bind_cols(.,pmap_df(list(lat = .$rounded_lat, lon = .$rounded_lon, zone = .$utm_zone), projfoo))
  }


ebs_locs %>% 
  ggplot() + 
  geom_sf(aes(fill = easting), shape = 21) + 
  scale_fill_viridis()
