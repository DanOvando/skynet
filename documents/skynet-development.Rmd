...
#   unnest()
# 
# b <- qmplot(rlon, rlat, color = total_mean_density, data = wtf) + 
#   facet_wrap(~year)

# a <- fish_maps %>% 
#   ungroup() %>% 
#   filter(rlon < 0, year > 2006) %>% 
...
ebsbts_trawlers <- gfw_data %>% 
  filter(survey == "ebsbts_gfw") %>% 
  unnest() %>% 
  filter(inferred_label_allyears == 'trawlers')
```

OK, all up and running. Now let's make sure that you're merging to knot data appropriately. The thing here is now you have densities by "knot", and those are in eastings and northings, which is a goddamn nightmare. Would really prefer to get these in units of lat-lon. 

```{r}

utm_coords <-
  SpatialDeltaGLMM::Convert_LL_to_UTM_Fn(
    Lon = ebsbts_trawlers$rounded_lon,
    Lat = ebsbts_trawlers$rounded_lat,
    zone = 32,
    flip_around_dateline = 1
  ) %>%
  dplyr::rename(e_km = X, n_km = Y) %>%
  select(e_km, n_km)

ebsbts_trawlers <- ebsbts_trawlers %>%
  bind_cols(utm_coords)


knots <- vast_alaska_pollock$spatial_densities %>%
  select(knot, e_km, n_km) %>%
  unique()
```

How the hell to deal with knots. What I'd like to get is a simple way to use the `sf` package for moving back and forth from UTM to lat-lon
...
a <- st_multipoint(as.matrix(knots), dim = 'XYM')

b <- st_sfc(a, crs =2077) #this turns it into an actual sfc object

d <- st_sf(data.frame(a = 1, b = b))

CRS("+proj=utm")

# A data frame of all the CRSs can be created:
# EPSG <- make_EPSG()
```
...
ebsbts_trawlers$knot <- knots$knot[nearest_knot$nn.idx]

ebsbts_trawlers$distance <- nearest_knot$nn.dists


knot_pairing_plot <- ebsbts_trawlers %>%
  filter(distance <= quantile(ebsbts_trawlers$distance, 0.75)) %>%
  select(e_km, n_km, knot) %>%
  unique() %>%
  ggplot() +
...
skynet_data <- ebsbts_trawlers %>%
  ...
  left_join(vast_alaska_pollock$spatial_densities, by = c('year', 'knot')) %>%
  ungroup() %>%
  mutate(
...
rf_model <- train(
  density ~ total_engine_hours +
    ...
    num_vessels +
    mean_vessel_length,
  data = skynet_sandbox$train[[1]],
  method = "rf",
  trControl = fitControl,
  do.trace = 10
)

skynet_sandbox$train[[1]]
```
