...
raw_arrow <- fish_data %>% 
  filter(survey == 'WCGBTS', Sci %in% c("Atheresthes_stomias", 'Microstomus_pacificus')) %>% 
  dplyr::rename(
    Lon = Long,
    Catch_KG = Wt,
    spp  = Sci
...
n_x = c(1000) # Number of knots

Kmeans_Config = list("randomseed" = 1,
                     "nstart" = 100, #the number of times that the k-means algorithm is run while searching for the best solution (default=100)
                     "iter.max" = 1e3)
...
strata_limits <- data.frame('STRATA' = "All_areas")

extrapolation_list <- SpatialDeltaGLMM::Prepare_Extrapolation_Data_Fn(Region = region, strata.limits = strata_limits)
...
raw_arrow <-  cbind(raw_arrow, "knot_i" = Spatial_List$knot_i)
...
tmb_data <-  Data_Fn(
  ...
  "RhoConfig" = RhoConfig,
  "ObsModel" = ObsModel,
  "c_i" = rep(0, nrow(raw_arrow)),
  "b_i" = raw_arrow[, 'Catch_KG'],
  "a_i" = raw_arrow[, 'AreaSwept_km2'],
  "v_i" = as.numeric(raw_arrow[, 'Vessel']) - 1,
  "s_i" = raw_arrow[, 'knot_i'] - 1,
  "t_iz" = raw_arrow[, 'Year'],
...
year_set <-  seq(min(raw_arrow[, 'Year']), max(raw_arrow[, 'Year']))

years_2_include <-  which(year_set %in% sort(unique(raw_arrow[, 'Year'])))
...
vast_arrow = cbind(
  ...
  arrange(knot, year) %>% 
  mutate(exp_density = exp(density))

SpatialDeltaGLMM::Plot_data_and_knots(
  Extrapolation_List = extrapolation_list,
  Spatial_List = spatial_list,
  Data_Geostat = raw_arrow,
  PlotDir = vast_file
  ) #plot map locations
  # check and make sure you're not hitting bounds, and that gradients are near 0
  DT::datatable(opt$diagnostics[, c('Param', 'Lower', 'MLE', 'Upper', 'final_gradient')])
  

encounter_prob <-
  SpatialDeltaGLMM::Check_encounter_prob(Report = report,
  Data_Geostat = raw_arrow,
  DirName = vast_file)
...
raw <- raw_arrow %>% 
  group_by(Year) %>% 
  summarise(abundance = sum(Catch_KG) * 100) %>% 
  mutate(source = 'raw')
...
assess_vast %>% 
  ggplot(aes(year, abundance, color = source)) + 
  geom_line()
