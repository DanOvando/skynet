...
fig_theme <- theme_ipsum(base_size = 14, axis_title_size = 16,subtitle_size = 12, axis_text_size = 14, plot_title_size = 12) + 
...
future_map <- future_performance %>%
  ggplot(aes(frame = year)) +
  geom_point(aes(x = recenter_lon, y = lat, fill = pmin(3,value)), show.legend = F, shape = 22, size = 5) +
  geom_sf(data = yearly_map, fill = 'grey60') +
  scale_fill_viridis() +
  coord_sf(xlim = c(bbox['xmin'], bbox['xmax']),
           ylim = c(bbox['ymin'], bbox['ymax'])) +
